...
void LCD_Show_Graph(u16 x,u16 y,u8 *num,u8 size,u8 mode)
{
	u8 temp;
	u16 t,t1;
	u16 y0 = y;
...
void LCD_draw_vline(u16 x0,u16 y0,u16 len,u16 width,u16 color)
{
        if(len==0)return;
        LCD_Fill(x0-width/2,y0,x0+width/2,y0+len-1,color);       
}
//direction:1代表上半圆，2代表下半圆
void LCD_fill_halfcircle(u16 direction,u16 width,u16 x0,u16 y0,u16 r,u16 color)
{                                                                                          
  u32 i;
...
//     	   LCD_draw_hline (x0-i+1,y0+x,2*(i-1),color);
					LCD_draw_hline (x0-i+1,y0-x,width,color);
					LCD_draw_hline (x0+i-width,y0-x,width,color);
				}
				x--;
			}
			// draw lines from inside (center)  
	//    LCD_draw_hline(x0-x,y0+i,2*x,color);
			LCD_draw_hline(x0-x,y0-i,width,color);
			LCD_draw_hline(x0+x-width,y0-i,width,color);
		}
	else
		for (i=1;i<=imax;i++)
		{
